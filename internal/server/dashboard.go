...
func (s *Server) handleDashboardAPI(w http.ResponseWriter, r *http.Request) {
	...
	s.dlService.GetAllTransfers(func(ctx *download.TransferContext) {
		// Accessor methods take the transfer lock internally.
		downloadedSize, totalSize, _, _ := ctx.GetProgress()

		// Only include downloading transfers with progress > 0
		if ctx.GetState() == download.TransferLifecycleDownloading && totalSize > 0 {
			downloadedMB := float64(downloadedSize) / 1024 / 1024
			totalMB := float64(totalSize) / 1024 / 1024
			progressPercent := (float64(downloadedSize) / float64(totalSize)) * 100
